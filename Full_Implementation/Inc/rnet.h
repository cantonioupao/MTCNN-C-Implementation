...
#ifndef __AI_RNET_H__
#define __AI_RNET_H__
#pragma once

#include "ai_platform.h"

#define AI_RNET_MODEL_NAME          "rnet"

#define AI_RNET_IN_NUM       (1)
#define AI_RNET_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 24, 24, 3, 1, NULL), \
}
#define AI_RNET_IN_SIZE { \
  (24 * 24 * 3), \
}
#define AI_RNET_IN_1_SIZE  (24 * 24 * 3)
#define AI_RNET_IN_1_SIZE_BYTES  ((24 * 24 * 3) * 4)




#define AI_RNET_OUT_NUM      (2)
#define AI_RNET_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 2, 1, NULL), \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 4, 1, NULL), \
}
#define AI_RNET_OUT_SIZE { \
  (1 * 1 * 2), \
  (1 * 1 * 4), \
}
#define AI_RNET_OUT_1_SIZE  (1 * 1 * 2)
#define AI_RNET_OUT_2_SIZE  (1 * 1 * 4)
#define AI_RNET_OUT_1_SIZE_BYTES  ((1 * 1 * 2) * 4)
#define AI_RNET_OUT_2_SIZE_BYTES  ((1 * 1 * 4) * 4)
 ...
 * @ref ai_rnet_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_RNET_*
 * defines into file @ref rnet.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
